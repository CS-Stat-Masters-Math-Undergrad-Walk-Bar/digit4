...
def worker(rank, world_size, mode, n_samples, batch_size, gpu_ids, shard_dir):
    ...
    shard_path = os.path.join(shard_dir, f'shard_{mode}_rank{rank}.pt')
...
def merge_shards(mode, world_size, shard_dir, out_dir):
    ...
    images_path = os.path.join(out_dir, f'generated_compose_{mode}.pt')
    scores_path = os.path.join(out_dir, f'scores_compose_{mode}.pt')
