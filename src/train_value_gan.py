...
emnist = torchvision.datasets.EMNIST(root=DATA_ROOT, split="digits", download=True, transform=transform_train)
...
VALUE_GAN_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
[os.remove(f) for f in VALUE_GAN_OUTPUT_DIR.glob('*.png')]
...
for epoch in trange(EPOCHS, desc="Epochs"):
    ...
    torch.save(generator.state_dict(), VALUE_GAN_GEN_PATH)
    torch.save(discriminator.state_dict(), VALUE_GAN_DISC_PATH)
    ...
    with torch.no_grad():
        ...
        torchvision.utils.save_image(grid, VALUE_GAN_OUTPUT_DIR / f"generated_samples_epoch_{epoch + 1}.png")
...
generated_samples[0].save(VALUE_GAN_OUTPUT_DIR / "progress.gif", save_all=True, append_images=generated_samples[1:], duration=200, loop=0)
