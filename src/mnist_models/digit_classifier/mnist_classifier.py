...
# %%
mnist_train_transforms = v2.Compose([
    v2.ToImage(),
...
def get_loaders():
    train_set = torchvision.datasets.MNIST(
        root=DATA_ROOT, train=True, download=True, transform=mnist_train_transforms
    )
...
# %%
def train(epochs=10, use_mixup=True, device=None):
    if device is None:
        device = get_device()
    ...
    for epoch in range(epochs):
        ...
        print(f"[MNIST] Epoch [{epoch+1}/{epochs}], Loss: {avg_loss:.4f}")
...
if __name__ == "__main__":
    ...
    train(epochs=10, use_mixup=True, device=device)
