...
for epoch in range(EPOCHS):
    ...
    if test_loss_epoch / len(test_loader) < smallest_test_loss:
        ...
        torch.save(model.state_dict(), HERE_STATE / 'best_cnn.pth')
    torch.save(model.state_dict(), HERE_STATE / 'last_cnn.pth')
...
model.load_state_dict(torch.load(HERE_STATE / 'best_cnn.pth'))
model.eval()
all_probs = []
all_labels = []
