...
HERE = ROOT / 'evaluation_presentation'
print('device:', device)
...
is_digit.load_state_dict(torch.load(
    ROOT / 'src/mnist_models/is_digit_binary_classifier/cnn/best_cnn.pth', map_location=device))
is_digit.eval()

digit_clf = build_digit_classifier().to(device)
digit_clf.load_state_dict(torch.load(
    ROOT / 'src/mnist_models/digit_classifier/emnist_mixup_classifier.pth', map_location=device))
digit_clf.eval()

disc = build_discriminator().to(device)
disc.load_state_dict(torch.load(
    ROOT / 'src/GAN/checkpoints/discriminators/discriminator_last.pth', map_location=device))
disc.eval()
...
# %%
METHODS = {
    'vae_interp':       HERE / 'vae_interp_samples.pt',
    'diffusion_avg':    ROOT / 'src/diffusion/v3_score/generated_compose_average.pt',
    'base_vae_latent_optim': HERE / "base_VAE_latent_optim.pt",
    'creative_vae_latent_optim': HERE / "creative_VAE_latent_optim.pt",
}

# Skip any method whose file is not yet on disk so the notebook stays runnable mid-pipeline
METHODS = {k: v for k, v in METHODS.items() if v.exists()}
...
torch.save({name: results[name]['scores'] for name in results},
           HERE / 'all_scores.pt')
print(f"\nsaved per-method score arrays -> {HERE / 'all_scores.pt'}")
...
plt.tight_layout()
fig.savefig(HERE / 'dc_mean_std_bar.png', dpi=130, bbox_inches='tight')
plt.show()
...
for name, r in results.items():
    ...
    fig_top.savefig(HERE / f'top16_{name}.png', dpi=130, bbox_inches='tight')
    plt.show()

    fig_rand = show_grid(rand_imgs, f'{name} — random 16', rand_scores, ncols=4)
    fig_rand.savefig(HERE / f'rand16_{name}.png', dpi=130, bbox_inches='tight')
    plt.show()
