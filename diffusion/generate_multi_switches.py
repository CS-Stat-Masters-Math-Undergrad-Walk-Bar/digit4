...
def run_experiments(checkpoint_path, out_dir, num_time_steps=1000, guidance_scale=3.0, n_samples=50):
    ...
    switch_counts = [1, 2, 4, 8, 10, 16, 20, 50, 70, 100]
    
    for switch_steps in switch_counts:
        print(f"Generating for switch_steps={switch_steps}...")
        with torch.no_grad():
            ...
            plt.tight_layout()
            out_path = os.path.join(out_dir, f'switch_step_{switch_steps}.png')
...
if __name__ == '__main__':
    ...
    out_dir = os.path.join(os.path.dirname(__file__), 'switch_experiments')
    run_experiments(ckpt, out_dir, n_samples=50)
